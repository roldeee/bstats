				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("GAMEID") === oObject.GAMEID;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].GAMEID;
										} else {
											sCurrentId = aItemsNotInTheList[0].GAMEID;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Games", {
											GAMEID : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("GAMEID"),
							title: oBindingContext.getProperty("GAMEID")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "Games (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'Games (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("GAMEID");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all Games to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "Games (" + iExpectedLength + ")"}),
									success : function () {
